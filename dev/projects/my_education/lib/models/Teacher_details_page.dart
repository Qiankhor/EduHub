...
class _TeacherDetailPageState extends State<TeacherDetailPage> with SingleTickerProviderStateMixin{
 late TabController _tabController;
  List<String> userBookedSessions = []; // To store the user's booked sessions
  final DatabaseReference _classRef = FirebaseDatabase.instance.ref().child('class');
  final DatabaseReference _sharingRef = FirebaseDatabase.instance.ref().child('sharing');
            ...
            builder: (BuildContext context) {
              return AlertDialog(
               title: Text('Join Successful'),
                content: Text('You have successfully joined. You can check the schedule for more details.'),
                actions: [
                  TextButton(
                    child: Text('OK'),
                    onPressed: () {
...
Future<void> _deductPoints(String userId, int points) async {
  ...
  DocumentReference providerRef = FirebaseFirestore.instance.collection('users').doc(widget.teacherData['provider']);
...
Future<bool> _showConfirmationDialog(int points) async {
  return await showDialog<bool>(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        ...
        content: Text('Submitting this will deduct $points points. Do you want to continue?'),
        actions: [
          TextButton(
            child: Text('Cancel'),
            onPressed: () {
  ...
  bool isSessionPast(String dateStr, String endTimeStr) {
    try {
      ...
      return DateTime.now().isAfter(
        DateTime(sessionDate.year, sessionDate.month, sessionDate.day, 23, 59, 59)
      );
    } catch (e) {
  ...
  Future<void> _fetchSharingClassData() async {
    try {
      ...
      final userDoc = await FirebaseFirestore.instance.collection('users').doc(widget.teacherData['provider']).get();
      final data = userDoc.data();
      ...
      for (String sessionId in createdSessions) {
        ...
        if (classSnapshot.exists) {
          Map<dynamic, dynamic> classItem = classSnapshot.value as Map<dynamic, dynamic>;
          ...
          if (isSessionPast(bookingData['date'] ?? '', bookingData['endTime'] ?? '')) {
        ...
        if (sharingSnapshot.exists) {
          Map<dynamic, dynamic> sharingItem = sharingSnapshot.value as Map<dynamic, dynamic>;
          Map<String, dynamic> bookingData = {
            'key': sessionId,
            'type': 'sharing',
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              if (!isSessionPast(item['date'] ?? '', item['endTime'] ?? ''))
                                Container(
                                  width: double.infinity,
                                  child: ElevatedButton(
                                  
                                    onPressed: () {
                                      _saveBooking(item['point'],item['id']);
                                    },
                                    ...
                                    child: Text('Join ('+item['point'].toString()+" points)",
