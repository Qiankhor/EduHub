...
class _QuizPageAdminState extends State<QuizPageAdmin> {
    ...
    try {
      ...
      if (categoriesSnapshot.exists) {
        ...
        for (var categoryEntry in categoriesData.entries) {
          ...
          Map<dynamic, dynamic> subjectsData = categoryEntry.value
              as Map<dynamic, dynamic>; // All subjects under the category

          List<QuizSet> fetchedQuizSets = [];

          for (var subjectEntry in subjectsData.entries) {
            String subjectName = subjectEntry.key; // Subject name
            Map<dynamic, dynamic> setsData = subjectEntry.value
                as Map<dynamic, dynamic>; // All sets under the subject

            for (var setEntry in setsData.entries) {
              String setName = setEntry.key; // Set name
              Map<dynamic, dynamic> setData =
                  setEntry.value as Map<dynamic, dynamic>; // Set data

              // Check the approval status
              if (setData.containsKey('approve') &&
  ...
  Future<void> _approveQuiz(String categoryName, String subjectName,
      String setName, String providerId) async {
    // 显示确认对话框
    ...
    if (confirmApproval == true) {
      try {
        ...
        await _quizRef
            .child('$categoryName/$subjectName/$setName')
            .update({'approve': true});
  ...
  Future<void> _sendApprovalMessage(
      String providerId, String category, String subject) async {
    try {
...
''';
  ...
  Future<void> _removeQuiz(String categoryName, String subjectName,
      String setName, String providerId) async {
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                  'Are you sure you want to reject this quiz? This action cannot be undone.'),
              SizedBox(height: 8),
              TextField(
    ...
    if (confirmRemoval == true) {
      try {
        ...
        await _sendRejectionMessage(
            providerId, reason, categoryName, subjectName);
        ...
        Navigator.of(context).pop(); // Close the loading dialog
        print('Error rejecting quiz set: $e');
      }
    }
  }

  Future<void> _sendRejectionMessage(
      String providerId, String reason, String category, String subject) async {
    try {
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      String categoryName =
                          quizSetsByCategory.keys.elementAt(index);
                      List<QuizSet> quizSets = quizSetsByCategory[categoryName]!
                          .where(
                              (quizSet) => quizSet.setData['approve'] == false)
                          .toList();

                      // If there are no quiz sets with approve == false, return a centered message
                      if (quizSets.isEmpty &&
                          quizSetsByCategory.values.every((sets) => sets.every(
                              (quizSet) =>
                                  quizSet.setData['approve'] == true))) {
                        return Center(
                          child: SizedBox(
                            height: MediaQuery.of(context)
                                .size
                                .height, // Set height to full height
                            width: MediaQuery.of(context)
                                .size
                                .width, // Set width to full width
                            child: Center(
                              child: Text(
                          ...
                          ? ExpansionTile(
                              ...
                              children: quizSets.map((quizSet) {
                                return Container(
                                  ...
                                  child: Padding(
                                    padding:
                                        EdgeInsets.fromLTRB(16, 12, 17, 11),
                                    child: Column(
                                      crossAxisAlignment:
