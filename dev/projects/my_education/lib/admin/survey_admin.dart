...
class _SurveyPageAdminState extends State<SurveyPageAdmin> with SingleTickerProviderStateMixin {
...
Future<void> _fetchData() async {
  try {
    ...
    if (snapshot.exists) {
      ...
      data.forEach((surveyId, surveyItem) {
        Map<dynamic, dynamic> surveyMap = Map<String, dynamic>.from(surveyItem);
        ...
        if (surveyMap.containsKey('proof')) {
          ...
          proofItems.forEach((proofKey, proofValue) {
            proofs.add({
              ...
              'targetAudience': surveyMap['targetAudience'] ?? 'Unknown Audience',
            });
          });
        }
      });
  ...
  Future<void> _approveSurvey(String key, String providerId, Map<String, dynamic> item) async {
...
Future<void> _removeSurvey(String key, String providerId, Map<String, dynamic> item) async {
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('Are you sure you want to reject this survey? This action cannot be undone.'),
            SizedBox(height: 8),
            TextField(
...
Future<void> _removeProof(BuildContext context, String provider, String surveyId, String proofId) async {
  TextEditingController _reasonController = TextEditingController();
                      ...
                      children: [
                       ElevatedButton(
                            ...
                            onPressed: () {
                              ...
                              _removeProof(
                                  context, proofItem['provider'], surveyId, proofId);
                            },
                            child: Text('Reject'),
                          ),
        ...
        child: Column(
          ...
          children: [
            Text(
              ...
              style: GoogleFonts.poppins(fontWeight: FontWeight.w700, fontSize: 14, color: Color(0xFF171A1F)),
            ),
            SizedBox(height: 4),
            Text(
            ...
            SizedBox(height: 4),
            Text('Provider: $providerName', style: GoogleFonts.manrope(fontSize: 12, color: Color(0xFF9095A0))),
            SizedBox(height: 8),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
...
void _showApproveDialog(BuildContext context, String provider, String surveyId, String proofId) {
  showDialog(
    context: context,
    builder: (context) {
      return AlertDialog(
...
Future<void> _approveProof(String providerId, String surveyId, String proofKey) async {
  try {
    showDialog(
        context: context,
        barrierDismissible: false, // 禁止在加载时关闭对话框
